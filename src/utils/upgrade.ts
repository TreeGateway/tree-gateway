...
export class VersionUpgrades {
    ...
    public async checkGatewayVersion(): Promise<void> {
    ...
    private async update(version: string) {
        if (compareVersions(version || '0.0.1', '3.0.0') < 0) {
            ...
            if (pac && pac.gateway) {
                ...
                if (!(pac.gateway as any)['disableStats']) {
                    pac.gateway.analytics = { enabled: true, logger: { name: 'redis' } };
                }
                pac.gateway = _.omit(pac.gateway, 'statsConfig', 'monitor', 'disableStats') as GatewayConfig;
            }
            if (pac.apis) {
                pac.apis = pac.apis.map(api => {
                    ...
                    return _.omit(api, 'proxy.statsConfig', 'proxy.disableStats',
                        ...
                        'proxy.interceptor') as ApiConfig;
                });
            }
